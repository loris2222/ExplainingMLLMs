...
class OwlViTModel(OwlViTPreTrainedModel):
    ...
    def forward(
        ...
        loss = None
        if return_loss:
            loss = owlvit_loss(logits_per_text)  # Pretraining contrastive loss
...
class OwlViTClassPredictionHead(nn.Module):
    ...
    def forward(
        ...
        # (?? This computes cosine similarity, as it is equivalent to tensordot 
        # over last dimension in image_class_embeds and query_embeds)
...
class OwlViTForObjectDetection(OwlViTPreTrainedModel):
    ...
    def image_text_embedder(
        ...
        pixel_values: torch.FloatTensor,
        attention_mask: torch.Tensor,
        custom_vision_tokens: Optional[torch.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
    ) -> Tuple[torch.FloatTensor]:
        ...
        if custom_vision_tokens is not None:
            image_embeds = custom_vision_tokens
        else:
            # Get image embeddings
            last_hidden_state = outputs.vision_model_output[0]  # This is outputs.vision_model_output.last_hidden_state
    ...
    def forward(
        ...
        query_embeds, feature_map, outputs = self.image_text_embedder(
            input_ids=input_ids,
            pixel_values=pixel_values,
            custom_vision_tokens=custom_vision_tokens,
            attention_mask=attention_mask,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        )
        ...
        batch_size, num_patches, num_patches, hidden_dim = feature_map.shape  # feature_map is image_embeds returned by image_text_embedder
        image_feats = torch.reshape(feature_map, (batch_size, num_patches * num_patches, hidden_dim))
